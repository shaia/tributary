...
// events are seen in the order the pipeline actually delivered them. The
// library deliberately has no per-event hook for this -- a std::function on
...
public:
    ...
    // Whether this sink was ever handed an event from producer `id`. The shard
...
// missed-wakeup test, which has to observe when an event arrives rather than
...
public:
    std::size_t write(std::span<const event> b) noexcept {
        ...
        // the count has also seen the events the count refers to.
...
// Accepts at most `limit` events per call, so the pipeline has to retain,
